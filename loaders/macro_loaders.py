...
def _coerce_int(value, default=0):
    """Parse integer values from XML attributes, including scientific notation."""
    if value is None:
        return default
    ...
    try:
        return builtins.int(builtins.float(str(value).strip()))
    except (TypeError, ValueError):
        return default


def _coerce_float(value, default=0.0):
    """Parse floating point values from XML attributes."""
    if value is None:
        return default
    ...
    try:
        return builtins.float(str(value).strip())
    except (TypeError, ValueError):
        return default
...
def macro_parser(macro_id, macro_type, prop_node, lresolver):
    ...
    if macro_type.startswith('ship_'):
        ...
        props['hull'] = _coerce_int(
            get_xpath_attrib(prop_node, './hull', 'max', -1), -1
        )
        ...
        props['boost_charge'] = float(boost.get('charge', 0))
        ...
        props['dock_external'] = _coerce_int(dock.get('external', 0))
        props['dock_capacity'] = _coerce_int(dock.get('capacity', 1))
        props['dock_allow'] = _coerce_int(dock.get('allow', 1))
        props['dock_storage'] = _coerce_int(dock.get('storage', 0))
        ...
        props['speed'] = _coerce_int(bullet.get('speed', 0))
        props['lifetime'] = float(bullet.get('lifetime', 0))
        props['range'] = _coerce_int(bullet.get('range', 0))
        props['amount'] = _coerce_int(bullet.get('amount', 0))
        props['barrelamount'] = _coerce_int(bullet.get('barrelamount', 0))
        props['timediff'] = float(bullet.get('timediff', 0))
        props['angle'] = float(bullet.get('angle', 0))
        props['maxhits'] = _coerce_int(bullet.get('maxhists', 0))
        props['ricochet'] = float(bullet.get('ricochet', 0))
        props['restitution'] = float(bullet.get('restitution', 0))
        props['scale'] = _coerce_int(bullet.get('scale', 0))
        props['attach'] = _coerce_int(bullet.get('attach', 0))
        props['chargetime'] = float(bullet.get('chargetime', 0))

        heat = get_xpath_attribs(prop_node, './heat', {})
        props['heat'] = _coerce_int(heat.get('value', 0))
        props['heat_initial'] = _coerce_int(heat.get('initia;', 0))
        ...
        props['dmg_hull'] = _coerce_int(damage.get('hull') or dmg_val)
        props['dmg_shields'] = _coerce_int(damage.get('shield') or dmg_val)
        props['dmg_min'] = _coerce_int(damage.get('min', -1))
        props['dmg_max'] = _coerce_int(damage.get('max', -1))
        props['dmg_repair'] = _coerce_int(damage.get('repair', 0))
        props['dmg_delay'] = _coerce_int(damage.get('delay', 0))

        mining_mult_xpath = './damage/multiplier[@mining]'
        props['dmg_mining_mult'] = _coerce_int(
